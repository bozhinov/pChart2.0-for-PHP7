...
class pDraw
{
	...
	*/
	...
	function pImagesetpixel($X, $Y, array $Color)
	{
		$this->Buff[] = [1, $X, $Y, 0, 0,$Color];
	}
	
	function pImageline($X, $Y, $XMin, $YMin, array $OuterBorderColor)
	{
		$this->Buff[] = [2, $X, $Y, $XMin, $YMin, $OuterBorderColor];
	}
	
	function pImageFilledPolygon($Points, $Num, array $Color)
	{
		$this->Buff[] = [3, $Points, $Num, 0, 0, $Color];
	}
		
	function pImagefilledrectangle($X1, $Y1, $X2, $Y2, array $Color)
	{
		$this->Buff[] = [4, $X1, $Y1, $X2, $Y2, $Color];
	}
	
	function pImagerectangle($X1, $Y1, $X2, $Y2, array $Color)
	{
		$this->Buff[] = [5, $X1, $Y1, $X2, $Y2, $Color];
	}
	
	function pImagettftext($FontSize, $Angle, $X, $Y, array $Color, $FontName, $Text)
	{
	...
	function drawPolygon($Points, array $Format = [])
	{
		...
		if (!$NoFill) {
			...
			$FillColor = $this->sanitizeColor($R, $G, $B, $Alpha);
			if (count($Points) >= 6) {
				$this->pImageFilledPolygon($Points, count($Points) / 2, $FillColor);
			}
		}
	...
	function drawRoundedRectangle($X1, $Y1, $X2, $Y2, $Radius, array $Format = [])
	{
		...
		if ($this->Antialias) {
			...
			$ColorA = $this->sanitizeColor($R, $G, $B, $Alpha);
			$this->pImageline($X1 + $Radius, $Y1, $X2 - $Radius, $Y1, $ColorA);
			$this->pImageline($X2, $Y1 + $Radius, $X2, $Y2 - $Radius, $ColorA);
			$this->pImageline($X2 - $Radius, $Y2, $X1 + $Radius, $Y2, $ColorA);
			$this->pImageline($X1, $Y1 + $Radius, $X1, $Y2 - $Radius, $ColorA);
		}
	...
	{		
		...
		$ManualColor = $this->sanitizeColor($R, $G, $B, $Alpha);
		foreach($Positions as $Yp => $Bounds) {
			...
			$this->pImageline($X1, $Yp, $X2, $Yp, $ManualColor);
		}
	...
	{		
		...
		if ($this->Antialias) {
			...
			$this->pImagerectangle($X1, $Y1, $X2, $Y2, $this->sanitizeColor($R, $G, $B, $Alpha));
		}
	}
	...
	function drawFilledRectangle($X1, $Y1, $X2, $Y2, array $Format = [])
	{
		...
		$Color = $this->sanitizeColor($R, $G, $B, $Alpha);
		if ($NoAngle) {
			$this->pImagefilledrectangle($X1c + 1, $Y1c, $X2f - 1, $Y2f, $Color);
			$this->pImageline($X1c, $Y1c + 1, $X1c, $Y2f - 1, $Color);
			$this->pImageline($X2f, $Y1c + 1, $X2f, $Y2f - 1, $Color);
		} else {
			$this->pImagefilledrectangle($X1c, $Y1c, $X2f, $Y2f, $Color);
		}
		...
		if ($Dash) {
			...
			$Color = $this->sanitizeColor($DashR, $DashG, $DashB, $Alpha);
			$Y = $iY1 - $DashStep;
			for ($X = $iX1; $X <= $iX2 + ($iY2 - $iY1); $X = $X + $DashStep) {
				...
				$this->pImageline($Xa, $Ya, $Xb, $Yb, $Color);
			}
		}
		...
		if ($this->Antialias && !$NoBorder) {
			if ($X1 < $X1c) {
				$AlphaA = $Alpha * ($X1c - $X1);
				$Color = $this->sanitizeColor($R, $G, $B, $AlphaA);
				$this->pImageline($X1c - 1, $Y1c, $X1c - 1, $Y2f, $Color);
			}

			if ($Y1 < $Y1c) {
				$AlphaA = $Alpha * ($Y1c - $Y1);
				$Color = $this->sanitizeColor($R, $G, $B, $AlphaA);
				$this->pImageline($X1c, $Y1c - 1, $X2f, $Y1c - 1, $Color);
			}

			if ($X2 > $X2f) {
				$AlphaA = $Alpha * (.5 - ($X2 - $X2f));
				$Color = $this->sanitizeColor($R, $G, $B, $AlphaA);
				$this->pImageline($X2f + 1, $Y1c, $X2f + 1, $Y2f, $Color);
			}

			if ($Y2 > $Y2f) {
				$AlphaA = $Alpha * (.5 - ($Y2 - $Y2f));
				$Color = $this->sanitizeColor($R, $G, $B, $AlphaA);
				$this->pImageline($X1c, $Y2f + 1, $X2f, $Y2f + 1, $Color);
			}
		}
	...
	function drawLine($X1, $Y1, $X2, $Y2, array $Format = []) # FAST
	{
		...
		if ($this->Antialias == FALSE && $Ticks == NULL) {
			if ($this->Shadow) {
				$ShadowColor = $this->sanitizeColor($this->ShadowR, $this->ShadowG, $this->ShadowB, $this->Shadowa);
				$this->pImageline($X1 + $this->ShadowX, $Y1 + $this->ShadowY, $X2 + $this->ShadowX, $Y2 + $this->ShadowY, $ShadowColor);
			}

			$Color = $this->sanitizeColor($R, $G, $B, $Alpha);
			$this->pImageline($X1, $Y1, $X2, $Y2, $Color);
			return;
		}
	...
	function drawFilledCircle(int $X, int $Y, int $Radius, array $Format = [])
	{
		...
		for ($i = 0; $i <= $Radius * 2; $i++) {
			...
			$this->Mask[$X + $XPos][$YPos] = TRUE;
			$this->pImageline($X - $XPos, $YPos, $X + $XPos, $YPos, $Color);
		}
	...
	function drawText(int $X, int $Y, string $Text, array $Format = [])
	{
		...
		if ($this->Shadow) {
			$C_ShadowColor = $this->sanitizeColor($this->ShadowR, $this->ShadowG, $this->ShadowB, $this->Shadowa);
			$this->pImagettftext($FontSize, $Angle, $X + $this->ShadowX, $Y + $this->ShadowY, $C_ShadowColor, realpath($FontName), $Text);
		}

		$C_TextColor = $this->sanitizeColor($R, $G, $B, $Alpha);
		$this->pImagettftext($FontSize, $Angle, $X, $Y, $C_TextColor, realpath($FontName), $Text);
		$this->Shadow = $Shadow;
	...
	function drawAntialiasPixel($X, $Y, array $Format = []) # FAST
	{
		...
		if (!$this->Antialias) {
			if ($this->Shadow) {
				$this->pImagesetpixel($X + $this->ShadowX, $Y + $this->ShadowY, $this->sanitizeColor($this->ShadowR, $this->ShadowG, $this->ShadowB, $this->Shadowa));
			}

			$this->pImagesetpixel($X, $Y, $this->sanitizeColor($R, $G, $B, $Alpha));
			return;
		}
	...
	function drawAlphaPixel($X, $Y, $Alpha, $R, $G, $B, $safe = FALSE) # FAST
	{
		...
		if ($this->Shadow) {
			$this->pImagesetpixel($X + $this->ShadowX, $Y + $this->ShadowY, $this->sanitizeColor($this->ShadowR, $this->ShadowG, $this->ShadowB, floor(($Alpha / 100) * $this->Shadowa)));
		}
	...
	function sanitizeColor($R, $G, $B, $Alpha = 100){
		if (!isset($this->aColorCache["$R.$G.$B.$Alpha"])){
			...
			$this->aColorCache["$R.$G.$B.$Alpha"] = [$R, $G, $B, 1.27 * (100 - $Alpha)];
		}
	...
	function drawArrow($X1, $Y1, $X2, $Y2, array $Format = [])
	{
		...
		$this->pImageFilledPolygon($Points, 4, $this->sanitizeColor($FillR, $FillG, $FillB, $Alpha));
	...
	function drawXThreshold(array $Values, array $Format = [])
	{
		...
		$CaptionSettings = [
			...
			"BoxBorderR" => $BoxR, # Momchil: must match drawThreshold
			"BoxBorderG" => $BoxG,
			"BoxBorderB" => $BoxB,
			"BoxBorderAlpha" => $BoxAlpha,
			"R" => $CaptionR,
	...
	function drawThreshold(array $Values, array $Format = [])
	{
		...
		$CaptionSettings = [
			...
			"BoxAlpha" => $BoxAlpha,
			"BoxSurrounding" => $BoxSurrounding,
			"BoxBorderR" => $BoxR, # Momchil: that was done to match the example
			"BoxBorderG" => $BoxG,
			"BoxBorderB" => $BoxB,
			"BoxBorderAlpha" => $BoxAlpha,
			"R" => $CaptionR,
	...
	function drawThresholdArea($Value1, $Value2, array $Format = []) 
	{
		...
		if ($this->myData->Data["Orientation"] == SCALE_POS_LEFTRIGHT) {
			...
			$YPos2 = $this->scaleComputeYSingle($Value2, ["AxisID" => $AxisID]);
			...
			$XPos1 = $this->scaleComputeYSingle($Value1, ["AxisID" => $AxisID]);
			$XPos2 = $this->scaleComputeYSingle($Value2, ["AxisID" => $AxisID]);
			...
			if ($Data["Orientation"] == SCALE_POS_LEFTRIGHT) {
				...
				foreach($SeriesName as $iKey => $SerieName) {
					...
					if (isset($Data["Series"][$SerieName]["Data"][$Index])) {
						...
						$Y = floor($this->scaleComputeYSingle($Value, ["AxisID" => $AxisID]));
						if ($Y < $MinY) {
							$MinY = $Y;
						}
				...
				foreach($SeriesName as $Key => $SerieName) {
					if (isset($Data["Series"][$SerieName]["Data"][$Index])) {
						...
						$X = floor($this->scaleComputeYSingle($Value,["AxisID" => $AxisID]));
	...
	function drawLabelBox($X, $Y, $Title, $Captions, array $Format = [])
	{
		...
		/* Outer border */
		$OuterBorderColor = $this->sanitizeColor(100, 100, 100, $BoxAlpha);
		$this->pImageline($XMin, $Y - 5, $X - 5, $Y - 5, $OuterBorderColor);
		$this->pImageline($X, $Y, $X - 5, $Y - 5, $OuterBorderColor);
		$this->pImageline($X, $Y, $X + 5, $Y - 5, $OuterBorderColor);
		$this->pImageline($X + 5, $Y - 5, $XMax, $Y - 5, $OuterBorderColor);
		if ($NoTitle) {
			$this->pImageline($XMin, $Y - 5 - $TitleHeight - $CaptionHeight - $HorizontalMargin * 2, $XMin, $Y - 5, $OuterBorderColor);
			$this->pImageline($XMax, $Y - 5 - $TitleHeight - $CaptionHeight - $HorizontalMargin * 2, $XMax, $Y - 5, $OuterBorderColor);
			$this->pImageline($XMin, $Y - 5 - $TitleHeight - $CaptionHeight - $HorizontalMargin * 2, $XMax, $Y - 5 - $TitleHeight - $CaptionHeight - $HorizontalMargin * 2, $OuterBorderColor);
		} else {
			$this->pImageline($XMin, $Y - 5 - $TitleHeight - $CaptionHeight - $HorizontalMargin * 3, $XMin, $Y - 5, $OuterBorderColor);
			$this->pImageline($XMax, $Y - 5 - $TitleHeight - $CaptionHeight - $HorizontalMargin * 3, $XMax, $Y - 5, $OuterBorderColor);
			$this->pImageline($XMin, $Y - 5 - $TitleHeight - $CaptionHeight - $HorizontalMargin * 3, $XMax, $Y - 5 - $TitleHeight - $CaptionHeight - $HorizontalMargin * 3, $OuterBorderColor);
		}

		/* Inner border */
		$InnerBorderColor = $this->sanitizeColor(255, 255, 255, $BoxAlpha);
		$this->pImageline($XMin + 1, $Y - 6, $X - 5, $Y - 6, $InnerBorderColor);
		$this->pImageline($X, $Y - 1, $X - 5, $Y - 6, $InnerBorderColor);
		$this->pImageline($X, $Y - 1, $X + 5, $Y - 6, $InnerBorderColor);
		$this->pImageline($X + 5, $Y - 6, $XMax - 1, $Y - 6, $InnerBorderColor);
		if ($NoTitle) {
			$this->pImageline($XMin + 1, $Y - 4 - $TitleHeight - $CaptionHeight - $HorizontalMargin * 2, $XMin + 1, $Y - 6, $InnerBorderColor);
			$this->pImageline($XMax - 1, $Y - 4 - $TitleHeight - $CaptionHeight - $HorizontalMargin * 2, $XMax - 1, $Y - 6, $InnerBorderColor);
			$this->pImageline($XMin + 1, $Y - 4 - $TitleHeight - $CaptionHeight - $HorizontalMargin * 2, $XMax - 1, $Y - 4 - $TitleHeight - $CaptionHeight - $HorizontalMargin * 2, $InnerBorderColor);
		} else {
			$this->pImageline($XMin + 1, $Y - 4 - $TitleHeight - $CaptionHeight - $HorizontalMargin * 3, $XMin + 1, $Y - 6, $InnerBorderColor);
			$this->pImageline($XMax - 1, $Y - 4 - $TitleHeight - $CaptionHeight - $HorizontalMargin * 3, $XMax - 1, $Y - 6, $InnerBorderColor);
			$this->pImageline($XMin + 1, $Y - 4 - $TitleHeight - $CaptionHeight - $HorizontalMargin * 3, $XMax - 1, $Y - 4 - $TitleHeight - $CaptionHeight - $HorizontalMargin * 3, $InnerBorderColor);
		}
		...
		if ($TitleMode == LABEL_TITLE_NOBACKGROUND && !$NoTitle) {
			...
			$this->pImageline($XMin + 1, $Y - 5 - $TitleHeight - $CaptionHeight - $HorizontalMargin + $HorizontalMargin / 2 + 1, $XMax - 1, $Y - 5 - $TitleHeight - $CaptionHeight - $HorizontalMargin + $HorizontalMargin / 2 + 1, $InnerBorderColor);
		}
	...
	function setFontProperties(array $Format = [])
	{
		...
		if ($FontName != NULL){
			if (!file_exists($FontName)){
				throw pException::InvalidResourcePath("Font path ".$FontName. " does not exist!");
			}
		}
	}
